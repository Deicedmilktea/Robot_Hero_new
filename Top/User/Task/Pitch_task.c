...
static void pitch_current_give()
{
    ...
    {
        if (vision_is_tracking)
            motor_top[5].set_current = pid_calc(&pitch.vision_pid_speed, pitch.target_speed, motor_top[5].rotor_speed);
        else
            motor_top[5].set_current = pid_calc(&pitch.pid_speed, pitch.target_speed, motor_top[5].rotor_speed);
    }
